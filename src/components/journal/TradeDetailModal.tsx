...
export default function TradeDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-xs animate-fade-in overflow-y-auto">
      <div 
        className="bg-white dark:bg-[#1E1E1E] border border-gray-200 dark:border-neutral-800 rounded-2xl w-full max-w-6xl max-h-[94vh] flex flex-col shadow-2xl overflow-hidden my-auto"
        ...
        <div className="p-4 sm:px-6 py-3.5 border-b border-gray-200 dark:border-neutral-800 flex items-center justify-between bg-gray-50/80 dark:bg-[#151515] shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-mono text-[#2563EB] dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded border border-blue-200 dark:border-blue-800/60">
                  {trade.session.replace('_', ' ')}
                </span>
              </div>
              <p className="text-[10.5px] text-gray-500 dark:text-neutral-400 font-mono">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 dark:bg-neutral-800 hover:bg-gray-200 dark:hover:bg-neutral-700 text-gray-700 dark:text-neutral-200 rounded-xl text-xs font-semibold transition-colors cursor-pointer border border-gray-200 dark:border-neutral-700 shadow-2xs"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/40 text-rose-700 dark:text-rose-400 border border-rose-200 dark:border-rose-800/60 rounded-xl text-xs font-semibold transition-colors cursor-pointer shadow-2xs"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 text-gray-400 dark:text-neutral-500 hover:text-gray-700 dark:hover:text-neutral-200 hover:bg-gray-100 dark:hover:bg-neutral-800 rounded-xl transition-colors cursor-pointer ml-1"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 flex-1 overflow-y-auto">
          ...
          <div className="lg:col-span-7 p-4 sm:p-6 bg-gray-50/50 dark:bg-[#151515] border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-neutral-800 flex flex-col justify-between">
            <div>
                ...
                {trade.chartUrl && (
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1 text-[11px] text-gray-600 dark:text-neutral-300 hover:text-[#2563EB] dark:hover:text-blue-400 bg-white dark:bg-[#181818] px-2 py-1 rounded-lg border border-gray-200 dark:border-neutral-800 transition-colors shadow-2xs"
                    >
                    ...
                    <a
                      ...
                      className="p-1 text-gray-600 dark:text-neutral-300 hover:text-[#2563EB] dark:hover:text-blue-400 bg-white dark:bg-[#181818] rounded-lg border border-gray-200 dark:border-neutral-800 shadow-2xs"
                      title="Open Original"
                    >
              ...
              <div className={`relative rounded-2xl overflow-hidden border border-gray-200 dark:border-neutral-800 bg-white dark:bg-[#181818] flex items-center justify-center transition-all shadow-2xs ${
                ...
                {trade.chartUrl ? (
                  ...
                  <div className="text-center p-8 text-gray-400 dark:text-neutral-500 space-y-2">
                    <ImageIcon className="w-12 h-12 mx-auto text-gray-300 dark:text-neutral-600" />
                    <p className="text-xs text-gray-600 dark:text-neutral-400">No chart screenshot attached for this trade.</p>
                    <p className="text-[10px] text-gray-400 dark:text-neutral-500">Click "Edit" to paste a TradingView snapshot link.</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    ...
                    className={`w-3.5 h-3.5 ${
                      star <= (trade.rating || 5) ? 'text-amber-400 fill-amber-400' : 'text-gray-200 dark:text-neutral-700'
                    }`}
                  />
                ))}
              </div>
          ...
          <div className="lg:col-span-5 p-4 sm:p-6 bg-white dark:bg-[#1E1E1E] space-y-5 overflow-y-auto">
            
            {/* Outcome Hero Banner */}
            <div className={`p-4 rounded-2xl border flex items-center justify-between shadow-xs ${
              isWin
                ? 'bg-gradient-to-r from-emerald-50/80 via-emerald-50/30 to-white dark:from-emerald-950/40 dark:via-[#181818] dark:to-[#181818] border-emerald-200 dark:border-emerald-800/60'
                : isLoss
                ? 'bg-gradient-to-r from-rose-50/80 via-rose-50/30 to-white dark:from-rose-950/40 dark:via-[#181818] dark:to-[#181818] border-rose-200 dark:border-rose-800/60'
              ...
              <div>
                ...
                <div className={`text-2xl font-mono font-extrabold ${
                  isWin ? 'text-emerald-600 dark:text-emerald-400' : isLoss ? 'text-rose-600 dark:text-rose-400' : 'text-gray-700 dark:text-neutral-300'
                }`}>
              ...
              <div className="bg-gray-50/80 dark:bg-[#151515] p-4 rounded-2xl border border-gray-200 dark:border-neutral-800 space-y-2.5">
                ...
                <div className="space-y-1.5 font-mono text-xs">
                  {trade.partials.map((p, i) => (
                    <div key={p.id || i} className="bg-white dark:bg-[#181818] p-2.5 rounded-xl border border-gray-200 dark:border-neutral-800 flex items-center justify-between shadow-2xs">
                      <div>
                        <span className="font-bold text-amber-700 dark:text-amber-400">Partial #{i + 1}: {p.percentage}%</span>
                        <span className="text-[10.5px] text-gray-500 dark:text-neutral-400 block font-sans">{p.note || 'Scaled at liquidity target'}</span>
                      </div>
                      <div className="text-right">
                        <span className="text-gray-900 dark:text-neutral-100 font-bold block">{p.exitPrice}</span>
                        <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold">+${p.pnl}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {trade.confluences.map(c => (
                    <span key={c} className="bg-gray-100 dark:bg-neutral-800 text-gray-700 dark:text-neutral-300 text-xs px-2.5 py-1 rounded-lg border border-gray-200 dark:border-neutral-700">
                      ✓ {c}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(trade.psychology || []).map(p => (
                    <span key={p} className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 text-[11px] font-medium px-2 py-0.5 rounded-lg border border-emerald-200 dark:border-emerald-800/60">
                      ★ {p}
                    </span>
                  ))}
